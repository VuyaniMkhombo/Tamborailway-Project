...
function loadMap(){
    mapboxgl.accessToken = 'pk.eyJ1IjoidmlzaDAwIiwiYSI6ImNqbXQ4azJjdTE3cngzcGw4azNvcjk1NzQifQ.RfdXjGHFHgTVLNZDARxFAQ';
    ...
    window.startPin = new mapboxgl.Marker(dragable = true).setLngLat([0, 0]).addTo(window.map)
    window.destinationPin = new mapboxgl.Marker(dragable = true).setLngLat([0, 0]).addTo(window.map)
    ...
    window.map.on('click', function(event){
        ...
        // code for getting the pin on the map
...
function loadButtonEvent(){
    //var backButton = dagencies-formocument.getElementById('back')
   // backButton.addEvenagencies-formtListener('click', function(event){
     //   event.preventDagencies-formefault()
    //   show('journey-form')
   // })
